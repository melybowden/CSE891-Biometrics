...
faces_folder_path = "../CelebAMask-HQ-mask-anno-short"
...
fp = open('failures.txt','w')
# Now process all the images
idx = 0
for f in glob.glob(os.path.join(faces_folder_path, "*.npz")):
    idx += 1
    if (idx % 100 == 0):
      print("idx: {}".format(idx))
    #print("Processing file: {}".format(f), end="\t")
    img = np.flip(np.load(f, allow_pickle=True)['Image'], -1)
    ...
    if(len(dets) != 1):
      #input() 
      fp.write("Processing file: {}\tNumber of faces detected: {}".format(f,len(dets))) 
    ...
    np.savez('./' + img_num + '.npz', face_descriptor)
